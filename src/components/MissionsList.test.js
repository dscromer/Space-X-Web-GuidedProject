...
test('re-renders correctly with the list of missions', () => {
    const {rerender, queryAllByTestId} = render(<MissionsList missions= {[]} />)

    const initialMissions = queryAllByTestId(/mission/i);
    expect(initialMissions).toHaveLength(0);
    expect(initialMissions).toStrictEqual([]);

    rerender(<MissionsList err='' missions={data} />);
    const missions = queryAllByTestId(/mission/i);
